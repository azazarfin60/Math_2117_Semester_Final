...
Thus $V = \begin{pmatrix} -1 \\ 2 \end{pmatrix}$ is an eigenvector corresponding to the eigenvalue $\lambda = 2$.

Again, let $W = \begin{pmatrix} w_1 \\ w_2 \end{pmatrix}$ be an eigenvector corresponding to the eigenvalue $\lambda = 5$.
...
then $\begin{pmatrix} 1 \\ 1 \end{pmatrix}$ is an eigenvector corresponding to the eigenvalue $\lambda = 5$.
...
**Consider the matrix $A = \begin{pmatrix} 3 & 1 \\ -1 & 1 \end{pmatrix}$**
...
Let, $V = \begin{pmatrix} v_1 \\ v_2 \end{pmatrix}$ be the eigenvector corresponding to the eigenvalue $\lambda = 2$.
...
<!-- Page 044 -->
Set $v_2 = 1 \Rightarrow v_1 = 1$? *(Note: In notes: $v_1-v_2 = 0 \Rightarrow v_1=v_2$, so $v_2=1 \Rightarrow v_1=1$. The eigenvector is $V = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$)*
$V = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$ is an eigenvector corresponding to the eigenvalue $\lambda = 2$.

we can't find independent eigenvectors. So we can't generate/find inverse matrix, also can't diagonalize matrix.

**# $\begin{pmatrix} 2 & 1 \\ -1 & 1 \end{pmatrix}$ HW eigen values and eigen vectors.**
...
**Consider the matrix $A = \begin{pmatrix} 1 & 2 & 3 \\ -1 & 2 & 4 \\ 1 & 1 & 2 \end{pmatrix}$**
...
<!-- Page 045 -->
$= (1-\lambda) \left\{ (2-\lambda)(2-\lambda) - 4 \right\} - 2 \left\{ -1(2-\lambda) - 4 \right\} + 3 \left\{ -1 - (2-\lambda) \right\}$
$= (1-\lambda) \left\{ 4 - 4\lambda + \lambda^2 - 4 \right\} - 2 \left\{ -2 + \lambda - 4 \right\} + 3 \left\{ -3 + \lambda \right\}$
...
So, $A^{-1} = \frac{1}{3}\begin{pmatrix} 0 & -1 & 2 \\ 6 & -1 & -7 \\ -3 & 1 & 4 \end{pmatrix}$ (found)
...
$\lambda = 2, 5$
$V = \begin{pmatrix} -1 \\ 2 \end{pmatrix}$, $W = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$

$P = \begin{pmatrix} -1 & 1 \\ 2 & 1 \end{pmatrix}$, $P^{-1} = \frac{1}{3}\begin{pmatrix} -1 & 1 \\ 2 & 1 \end{pmatrix}$
...
$\mathbb{R}^2 = \{(a, b) \mid a, b \in \mathbb{R}\}$ such that vector $(1, 2)$ is a member of $\mathbb{R}^2$.
...
$\mathbb{R}^4 = \{(a, b, c, d) \mid a, b, c, d \in \mathbb{R}\}$
আর ভেক্টর আকারে প্রকাশ করা যাবে না।

$\mathbb{R}^n = \{(a_1, a_2, \dots, a_n) \mid a_1, a_2, a_3, \dots, a_n \in \mathbb{R}\}$
(Row vector)
...
<!-- Page 048 -->
$\mathbb{R}^2 \rightarrow \left. \begin{matrix} (1, 0) \\ (0, 1) \end{matrix} \right\} \text{independent vector (user basis)}$
...
$A = \begin{pmatrix} 1 & 2 & 3 \\ 4 & 5 & 6 \\ 7 & 8 & 9 \end{pmatrix}$, $B = \begin{pmatrix} 2 & 3 & 1 \\ 0 & 1 & 2 \\ 1 & 1 & -1 \end{pmatrix}$
...
- $\{(1, 2), (2, 3)\}$ it is not maintaining closure property.
- $\mathbb{R}^n = \{(a_1, a_2, a_3 \dots a_n) \mid a_1, a_2 \dots \in \mathbb{N}\}$
- $(a, b, c) = a\hat{i} + b\hat{j} + c\hat{k}$
- $\{(0, 0)\}$ satisfy the closure property.
...
**Binary operation:**
$f: A \otimes A \rightarrow A \quad (R = \{ \dots \})$
...
4. **Identity law:**
   ...
   - $\mathbb{Z} = \{\dots, -2, -1, 0, 1, 2, \dots \}$ (identity for $\mathbb{Z}$)