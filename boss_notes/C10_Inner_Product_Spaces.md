...
**Proof**:
Let $V$ be an inner product space. We define the induced norm as $\|x\| = \sqrt{\langle x, x \rangle}$. We must prove that this satisfies the three axioms of a norm.

1.  **Positivity**: By the positive-definite axiom of inner products, $\langle x, x \rangle \geq 0$. Thus, $\|x\| = \sqrt{\langle x, x \rangle} \geq 0$. Additionally, $\langle x, x \rangle = 0 \iff x = 0$, so $\|x\| = 0 \iff x = 0$.
...
3.  **Triangle Inequality**:
    ...
    By the Cauchy-Schwarz inequality ($\lvert \langle x, y \rangle \rvert \leq \|x\| \|y\|$) and knowing $\text{Re}(z) \leq \lvert z \rvert$:
