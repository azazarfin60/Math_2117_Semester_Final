...
$$
(-1)^{n-1} \lambda^{n-1} (a_{11} + a_{22} + \dots + a_{nn}) = (-1)^{n-1} \lambda^{n-1} \operatorname{tr}(A)
$$
Since both expressions must be equal, the coefficients of $\lambda^{n-1}$ are identical:
$$
(-1)^{n-1} \sum_{i=1}^n \lambda_i = (-1)^{n-1} \operatorname{tr}(A)
$$
...
Construct a diagonal matrix $D$ with the corresponding eigenvalues on the diagonal: $D = \operatorname{diag}(\lambda_1, \lambda_2, \dots, \lambda_n)$.
...
Similarly, $PD = \begin{pmatrix} v_1 & v_2 & \dots & v_n \end{pmatrix} \operatorname{diag}(\lambda_1, \dots, \lambda_n) = \begin{pmatrix} \lambda_1 v_1 & \lambda_2 v_2 & \dots & \lambda_n v_n \end{pmatrix}$.
