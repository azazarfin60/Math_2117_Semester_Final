...
**Answer:**
Let $V$ be a finite-dimensional vector space. Let $B_1 = \{u_1, u_2, \dots, u_m\}$ and $B_2 = \{v_1, v_2, \dots, v_n\}$ be two bases for $V$. We want to show that $m = n$.
...
**Answer:**
Let $V$ be an $n$-dimensional vector space. Let $S = \{v_1, v_2, \dots, v_k\}$ be a linearly independent set in $V$ with $k < n$. We want to extend $S$ to form a basis for $V$.

Let $B = \{e_1, e_2, \dots, e_n\}$ be any known basis for $V$ (such as the standard basis if $V = \mathbb{R}^n$).
Consider the union of sets: $S \cup B = \{v_1, \dots, v_k, e_1, \dots, e_n\}$.
