...
$$\begin{array}{l|r}
\text{c(u+v) = cv + cu} & \{ (0,0) \} \rightarrow \text{Vector space over a field } \mathbb{R}. \\
\text{(c+k) u = cu + ku} & \{ (2,1) \} \rightarrow \text{Not vector space} \\
\text{(ck) u = c (ku)} & \{ (2,1), (4,2) \} \rightarrow \text{Not vector space} \\
\text{1u = u} & 
\end{array}$$
...
$\star$ **$\{ (x,y) : y = mx \}$ is a vector space?**
...
$\star$ **$\mathbb{R}^n = \{ (a\_1, a\_2 \dots a\_n) : a\_i \in \mathbb{R} \}$ and $F=\mathbb{R}$**
...
$$\# \ \{ (1,2) \} = V \rightarrow \text{not a vector space.}$$
$$\# \ \{ (1,2), (0,0), (-1,-2) \} \rightarrow \text{Not a vector space because satisfies all the properties of addition but does not satisfy the multiplication properties.}$$
