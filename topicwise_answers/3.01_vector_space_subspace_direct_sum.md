...
*   **Direct Sum of Subspaces:** Let $U$ and $W$ be subspaces of $V$. The sum $U + W$ is a direct sum (written $U \oplus W$) if every element in the sum can be written uniquely as $u + w$. This occurs if and only if $U \cap W = \lbrace 0\rbrace$.
    *Example:* In $\mathbb{R}^2$, let $U$ be the $x$-axis and $W$ be the $y$-axis. Their intersection is only the origin $\lbrace(0, 0)\rbrace$. So $U \oplus W = \mathbb{R}^2$.
...
A subset $B = \lbrace v\_1, v\_2, \dots, v\_n\rbrace$ of a vector space $V$ is a basis of $V$ if:
