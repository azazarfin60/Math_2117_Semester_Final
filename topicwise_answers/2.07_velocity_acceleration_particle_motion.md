...
If $\bar{A}$ has a constant magnitude show that $\bar{A}$ and $\frac{d\bar{A}}{dt}$ are perpendicular provided
...
Show that the velocity $\vec{v}$ of the particle is perpendicular to $\vec{r}$, also show that $\vec{r} \times \vec{v} = \vec{a}$ constant vector.
